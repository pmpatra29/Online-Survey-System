...
public class SQLManage {
	...
	public boolean check(String search) throws SQLException {
        ...
        public int getMaxQuestionNumber(String surveyCode) throws SQLException {
            int maxQuestionNumber = 0;
            String query = "SELECT MAX(qno) AS max_qno FROM survey.surveyquestions WHERE surveycode = ?";
            try (PreparedStatement statement = con.prepareStatement(query)) {
                statement.setString(1, surveyCode);
                ResultSet resultSet = statement.executeQuery();
                if (resultSet.next()) {
                maxQuestionNumber = resultSet.getInt("max_qno");
                }
            }
